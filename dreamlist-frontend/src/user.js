function getUserData() {
  return fetch(`http://localhost:3000/users`)
    .then(res => res.json())
    .then(data =>
      data.filter(user => user.id === parseInt(sessionStorage.user_id))
    );
...
function addVacationData(data) {
  ...
  return fetch(`http://localhost:3000/vacations`, options).then(res =>
    res.json()
  );
}
...
async function renderUserInfo(user) {
  ...
  userName.innerHTML = `<h1>${user[0].name} 👻</h1> from ${await getHomebase(
    user[0].home_base_id
  )}`;
...
function renderVacations(vacations) {
  ...
  vacations.forEach(vacation => {
    ...
    btn.addEventListener("click", e => {
      deleteVacationData(vacation.id).then(() =>
        getUserData().then(user => renderVacations(user[0].vacations))
      );
    });
    td.appendChild(btn);
